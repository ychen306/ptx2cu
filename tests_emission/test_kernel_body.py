...
def test_emit_kernel_simple():
    ...
    kernel = CudaKernel(
        ...
        body=[
            Load(bitwidth=32, is_float=False, dst=Var("r0", 32, False), src=Var("p0", 32, False), offset=0),
            CudaLabel(name="L0"),
            InlineAsm(template="add.s32 %0, %1, %2;", arguments=[Var("r0", 32, False), Var("r0", 32, False), Var("r0", 32, False)], outputs=[Var("r0", 32, False)]),
