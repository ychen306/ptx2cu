...
def test_emit_branch_predicated():
    ...
    regmap = {pred: Var("p1", tpred)}
    out = emit_branch(br, regmap)
    assert out == CudaBranch(cond=Var("p1", tpred), target=CudaLabel(name="L2"))
...
def test_emit_branch_predicated_without_percent():
    ...
    regmap = {pred: Var("p", tpred)}
    out = emit_branch(br, regmap)
    assert out == CudaBranch(cond=Var("p", tpred), target=CudaLabel(name="L3"))
