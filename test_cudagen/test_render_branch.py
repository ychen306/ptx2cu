...
def test_emit_branch_predicated():
    ...
    regmap = {pred: Var("p1", 32, False, True)}
    out = emit_branch(br, regmap)
    assert out == CudaBranch(
        cond=Var("p1", 32, False, True), target=CudaLabel(name="L2")
    )
...
def test_emit_branch_predicated_without_percent():
    ...
    regmap = {pred: Var("p", 32, False, True)}
    out = emit_branch(br, regmap)
    assert out == CudaBranch(
        cond=Var("p", 32, False, True), target=CudaLabel(name="L3")
    )
